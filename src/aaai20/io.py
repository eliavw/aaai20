...
def build_filesystem(root=None):
    """
    root
    |---config
    |   |---query
    ...
    if root is None:
        fs["root"] = dirname(dirname(dirname(__file__)))
    else:
        fs["root"] = root

    # Level 01
    fs["config"] = os.path.join(fs["root"], "config")
    fs["data"] = os.path.join(fs["root"], "data")
    fs["out"] = os.path.join(fs["root"], "out")
    fs["cli"] = os.path.join(fs["root"], "cli")

    # Level 02
    fs["query"] = os.path.join(fs["config"], "query")
    fs["model"] = os.path.join(fs["config"], "model")
    fs["fit"] = os.path.join(fs["cli"], "fit")
    fs["predict"] = os.path.join(fs["cli"], "predict")
    fs["exe"] = os.path.join(fs["cli"], "exe")
    fs["cli-config"] = os.path.join(fs["cli"], "cli-config")

    # out-folders premade
    fs["run_mercs"] = os.path.join(fs["out"], "run_mercs")
    fs["run_pxs"] = os.path.join(fs["out"], "run_pxs")
    fs["fit_pxs"] = os.path.join(fs["out"], "fit_pxs")
    fs["fit_mercs"] = os.path.join(fs["out"], "fit_mercs")

    fs["raw"] = os.path.join(fs["data"], "raw")

    # Level 03
    fs["datasets-UCI"] = os.path.join(fs["raw"], "datasets-UCI")
    fs["datasets-starai"] = os.path.join(fs["raw"], "datasets-starai")
...
def exp_directory(exp_dname="default", script=None, fs=None):
    if script is None:
        script = "manual"
