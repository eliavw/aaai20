...
from .log.monitor import Logfile, TimeLimit
from .log.process import Process
...
# Run
def run_script(
    script_fname, config_fname, log_fname, fold=None, timeout=None, q_idx=None
):
    """
...
def generate_monitor(log_fname, timeout):
    ...
    timeout is:     {}\n
    """.format(
        __file__, log_fname, timeout
    )
    debug_print(msg, V=VERBOSITY)

    monitors = [Logfile(log_fname), TimeLimit(timeout)]
